#!/usr/bin/env python3
import os, sys, subprocess, shutil, re, time, threading, signal, textwrap,time
from shutil import get_terminal_size

# --- INSTALL MODULE ---
try:
    os.system("apt install binutils ")
...
def apply_patch(work_dir, mode, target_lang='id'):
    ...
    "Lcom/google/android/gms/ads/AdRequest$Builder;->build",
    "Lmt/modder/hub/MTMain;->main",
    "Lmt/modder/hub/Check;->Verify",
    "invoke-static {p0}, Lcom/google/android/gms/ads/MobileAds;->initialize",
    ...
    if mode == 7:
        ...
        for s_dir in [d for d in os.listdir(work_dir) if d.startswith("smali")]:
            for folder in ["Lmt/modder/hub/","mt/modder/hub", 
            "mt/modder", 
    ...
    for root, _, files in os.walk(work_dir):
        if any(x in root for x in ["androidx", "com/google", "mt/modder","com/google/android/gms/ads",
            "com/google/ads",
            "com/applovin",
            "com/facebook/ads",
            "com/unity3d/ads",
